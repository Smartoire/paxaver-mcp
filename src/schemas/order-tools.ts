...
export const orderTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false, title: 'Get Daily Orders (Admin)' },
  },
  {
  ...
  {
    name: 'get-published-menu',
    description:
      'Returns the published daily lunch menu for the user\'s active school. Accepts either \'date\' (YYYY-MM-DD) or \'month\' (YYYY-MM). If neither is given, returns today\'s menu. Read-only. Use this to find menu_item_id values for create-draft-order.',
    inputSchema: {
      type: 'object',
      properties: {
        date: { type: 'string', description: 'YYYY-MM-DD' },
  ...
  },
  {
    ...
    description:
      'Creates a draft lunch order for a student. Requires student_id, school_slug, menu_date, and items array. Each item needs menu_item_id, menu_item_name, price_cents, and quantity. The draft is not finalized — call finalize-order to commit the order and deduct payment. This is a FINANCIAL + WRITE operation — always confirm the order details with the user before calling. Idempotent.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        order_id: { type: 'string', description: 'Order ID from create-draft-order' },
