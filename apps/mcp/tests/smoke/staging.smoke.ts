...
describe('staging smoke (read-only)', () => {
  it('health endpoint returns 200', async () => {
    ...
    const json = (await res.json()) as unknown as { status: string; environment: string };
  ...
  it('protected resource metadata is valid', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as unknown as { resource: string; scopes_supported: string[] };
